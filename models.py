...
class BiDAFCoattended(nn.Module):
    """Coattention adapted BiDAF model for SQuAD.
    ...
    def __init__(self, word_vectors, hidden_size, drop_prob=0.):
        ...
        # Replace initial attention with co-attention                             
...
class BiDAFSelfAttended(nn.Module):
    """Baseline BiDAF model for SQuAD.
    ...
    """
    def __init__(self, word_vectors, hidden_size, drop_prob=0.):
        super(BiDAFSelfAttended, self).__init__()
        self.emb = layers.Embedding(word_vectors=word_vectors,
                                    hidden_size=hidden_size,
                                    drop_prob=drop_prob)
...
class BiDAFCombined(nn.Module):
    """
    BiDAF Combined model which uses character embeddings, data augmentation and our best performing self-attention.
    Args:
        word_vectors (torch.Tensor): Pre-trained word vectors.
        hidden_size (int): Number of features in the hidden state at each layer.
        drop_prob (float): Dropout probability.
    ...
    def forward(self, cw_idxs, qw_idxs, cc_idxs, qc_idxs):
        ...
        if self.use_char_emb:
            ...
            c_emb = self.emb(cw_idxs, None)
            q_emb = self.emb(qw_idxs, None)
